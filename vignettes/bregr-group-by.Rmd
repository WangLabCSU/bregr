...
We are using the same example from the [ezcox package vignette](https://cran.r-project.org/web/packages/ezcox/vignettes/ezgroup.html).
...
```{r show-forest, fig.width=8, fig.height=4, dpi=150}
br_show_forest(mds)
```
...
```{r show-forest2, fig.width=7, fig.height=4, dpi=150}
br_show_forest(
  mds,
  drop = 2,
...
To subset the data rows, we can input an R expression using variables from `br_get_results(mds)`. For example, we can use `Group_variable == "2" & variable == "ph.ecog" & label == 3` to locate the row we want to remove, and then use `!()` to select the negated rows.

If drop `All` group is necessary, update the `subset` with:

```{r show-forest-rm-all, fig.width=7, fig.height=4, dpi=150}
br_show_forest(
  mds,
  drop = 2,
  subset = !((Group_variable == "2" & variable == "ph.ecog" & label == 3) |
